...
Comme vu ci-dessus, une API tRPC a deux composants principaux, [`schema`](#schema) et [`backend`](#backend), définis comme des projets distincts dans votre espace de travail.
...
- `publicProcedure` définit une méthode publique de l'API, incluant le middleware configuré dans `src/middleware`. Ce middleware inclut l'intégration d'AWS Lambda Powertools pour le logging, tracing et les métriques.
