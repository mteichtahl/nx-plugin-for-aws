...
export class MyApi extends Construct {
  ...
  constructor(scope: Construct, id: string) {
    ...
    // Enregistre l'URL dans la configuration runtime pour la découverte client
...
Pour consommer un stream de réponses, utilisez le <Link path="guides/api-connection/react-fastapi#consuming-a-stream">Générateur API Connection</Link> qui fournira une méthode type-safe pour itérer sur les chunks.

## Déploiement de votre FastAPI

Le générateur FastAPI crée un construct CDK pour déployer votre API dans `common/constructs`. Vous pouvez l'utiliser dans une application CDK :
...
1. Une fonction AWS Lambda par opération dans l'API  
...
:::tip
Si vous travaillez simultanément sur l'infrastructure CDK et l'API FastAPI, utilisez [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) pour régénérer les types automatiquement :
...
Si vous avez choisi l'authentification `IAM`, utilisez `grantInvokeAccess` pour octroyer l'accès :
