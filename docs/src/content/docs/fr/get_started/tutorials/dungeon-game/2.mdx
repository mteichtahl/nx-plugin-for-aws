...
Nous allons implémenter notre base de données dans DynamoDB en utilisant la bibliothèque cliente [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/). Pour commencer, installons d'abord `electrodb` avec la commande :
...
Augmentation de `IMiddlewareContext` pour ajouter `IDynamoDBContext`.
  </TabItem>
...
:::note
Notez que chaque procédure étant servie par une fonction lambda distincte, nous pouvons appliquer le principe du moindre privilège en n'accordant que les permissions nécessaires selon l'implémentation de chaque procédure.
:::
  </TabItem>
</Tabs>
...
**Non recommandé** car cela pourrait déployer des stacks indésirables si vous avez des environnements séparés (ex: `infra-prod`).
...
Ajoutez ce script à votre `.bashrc` ou collez-le dans le terminal :
```bash
// ~/.bashrc
acurl () {
    REGION=$1
...
```
    <Aside type="caution">
    Utilisez la valeur de sortie CDK `dungeon-adventure-infra-sandbox.GameApiGameApiEndpointXXX` et ajustez la région.
    </Aside>
  </TabItem>
</Tabs>
