...
Nous allons implémenter notre base de données dans DynamoDB en utilisant la bibliothèque cliente [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/) pour simplifier les opérations. Pour commencer, installons `electrodb` avec la commande :
...
:::note
Comme chaque procédure est servie par une fonction lambda distincte, nous pouvons appliquer le principe de moindre privilège en accordant uniquement les permissions nécessaires (lecture/écriture) selon l'implémentation de la procédure.
:::
  </TabItem>
</Tabs>
...
<Aside type="tip">
En cas d'erreurs de linting, exécutez cette commande pour les corriger automatiquement :
...
<Drawer title="Commande de déploiement" trigger="Vous pouvez aussi déployer toutes les stacks d'un coup. Cliquez pour plus de détails.">

Déployez toutes les stacks de l'application CDK avec :

<NxCommands commands={['run @dungeon-adventure/infra:deploy --all']} />

**Non recommandé** si vous avez des stacks séparées (ex: `infra-prod`), car le flag `--all` déploierait tout, y compris les environnements non souhaités !

</Drawer>

Une fois déployé, vous verrez des sorties similaires à ceci _(valeurs masquées)_ :
...
```
    <Aside type="caution">
    Utilisez la sortie CDK `dungeon-adventure-infra-sandbox.GameApiGameApiEndpointXXX` pour l'URL et ajustez la région.
    </Aside>
  </TabItem>
</Tabs>

:::note
Le `%7B%7D` passé à l'API est un objet JSON vide encodé URI (`{}`).
:::
