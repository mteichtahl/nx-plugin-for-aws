...
Démontrons les capacités de `@tanstack/react-router` en créant une route typée. Créez un fichier vide à l'emplacement : `packages/game-ui/src/routes/game/index.tsx`. Observez les logs du serveur :
...
Naviguez vers `http://localhost:4200/game` pour voir la nouvelle page !

<Image src={baselineGamePng} alt="baseline-game.png" width="800" height="600" />

Mettons à jour `index.tsx` pour charger `/game` par défaut. Notez comment le champ `to` propose des routes typées :
        ...
        utilities={[
          {
            ...
            items: [{ id: 'signout', text: 'Se déconnecter' }],
          },
        ]}
      />
      <Outlet />
...
Supprimez les fichiers `packages/game-ui/src/components/AppLayout/navitems.ts` et `packages/game-ui/src/hooks/useAppLayout.tsx`.
    ...
    <div className="game-interface">
      ...
      <div className="input-area">
        <PromptInput
          ...
          value={currentInput}
          actionButtonAriaLabel="Envoyer"
          actionButtonIconName="send"
          ariaLabel="Champ de saisie"
          placeholder="Quelle action entreprenez-vous ?"
...
<Aside type="tip">
La syntaxe `$playerName` indique à `@tanstack/react-router` de traiter `playerName` comme un [paramètre de chemin](https://tanstack.com/router/v1/docs/framework/react/guide/path-params). La méthode `validateSearch` assure que le paramètre `genre` est typé selon notre enum.
</Aside>
</TabItem>
</Tabs>
...
<Drawer title="Build et Déploiement" trigger="Vous pouvez aussi déployer sur Cloudfront">
