...
Ce guide vous explique les bases de l'installation et de l'utilisation de `@aws/nx-plugin` pour construire rapidement des projets sur AWS.

:::tip
Pour un tutoriel plus détaillé sur la construction d'une application full-stack, consultez le <Link path="get_started/tutorials/dungeon-game/overview">Tutoriel Dungeon Adventure</Link>.
:::
...
Cela génère automatiquement un nouveau site React dans `packages/demo-website`.
