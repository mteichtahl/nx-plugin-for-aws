...
La StoryApi consiste in un'unica API `generate_story` che, dato un `Game` e una lista di `Action` come contesto, farà progredire una storia. Questa API sarà implementata come API streaming in Python/FastAPI e dimostrerà inoltre come modificare il codice generato per adattarlo allo scopo.
...
Ora sostituiamo il contenuto di `packages/story_api/story_api/main.py` con quanto segue:
...
- Utilizziamo l'impostazione `x-streaming` per indicare che questa è un'API streaming quando genereremo il nostro client SDK. Questo ci permetterà di consumare questa API in modalità streaming mantenendo la type-safety!
...
:::note
Ogni volta che modifichi la tua FastAPI, dovrai ricostruire il progetto per vedere le modifiche riflesse nel client generato nel tuo sito web.

Faremo altre modifiche di seguito prima di ricostruire.
:::
...
export class StoryApi extends Construct {
  ...
  constructor(scope: Construct, id: string) {
    ...
    // Registra l'URL API nella configurazione runtime per la scoperta client
...
export class ApplicationStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // Concede permessi di lettura/scrittura a ciascun handler in base ai requisiti
...
+    );
    ...
    // concede al ruolo autenticato l'accesso per invocare le API
    [storyApi, gameApi].forEach((api) =>
      api.grantInvokeAccess(userIdentity.identityPool.authenticatedRole),
    );

    // Istanzia per ultimo per garantire la corretta configurazione automatica di runtime-config.json
...
Ora aggiorneremo lo `story_api` per supportare il deployment con [Lambda Web Adapter](https://github.com/awslabs/aws-lambda-web-adapter).
...
<Drawer title="Comando di deployment" trigger="Puoi anche deployare tutti gli stack insieme. Clicca per maggiori dettagli.">

Puoi deployare tutti gli stack dell'applicazione CDK eseguendo:

<NxCommands commands={['run @dungeon-adventure/infra:deploy --all']} />

**Non raccomandato** poiché potresti voler separare gli stage di deployment in stack diversi (es. `infra-prod`). In questo caso il flag `--all` tenterà di deployare tutto, rischiando deployment indesiderati!
...
<li>
<Drawer title="Curl con Sigv4 abilitato" trigger="Chiamare l'API deployata direttamente con curl abilitato per sigv4">
Puoi aggiungere questo script al tuo file `.bashrc` (e fare `source`) o incollarlo direttamente nel terminale.
```bash
// ~/.bashrc
acurl () {
    REGION=$1
...
Esempi di chiamata autenticata con sigv4:
...
```
    <Aside type="caution">
    Utilizza il valore `dungeon-adventure-infra-sandbox.StoryApiStoryApiUrlXXX` dall'output del CDK per sostituire il placeholder dell'URL e imposta la regione corretta.
    </Aside>
  </TabItem>
</Tabs>

Se il comando ha successo, vedrai una risposta in streaming simile a:

```
UnnamedHero si ergeva maestoso, il mantello sventolava nel vento....
```

Congratulazioni. Hai creato e deployato la tua prima API con FastAPI! 🎉🎉🎉