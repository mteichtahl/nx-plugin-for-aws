...
<Aside>
모든 종속성은 [단일 버전 정책](https://nx.dev/concepts/decisions/dependency-management#single-version-policy) 원칙을 따르는 `@aws/nx-plugin`에 따라 루트 `package.json`에 추가됩니다. 자세한 내용은 <Link path="guides/typescript-project#dependencies">ts#project 가이드</Link>를 참조하세요.
</Aside>
...
ElectroDB는 매우 강력하여 타입 정의뿐만 아니라 위의 타임스탬프와 같은 특정 값에 대한 기본값을 제공할 수 있습니다. 또한 ElectroDB는 DynamoDB 사용 시 권장되는 [단일 테이블 설계](https://electrodb.dev/en/core-concepts/single-table-relationships/)를 따릅니다.
...
각 프로시저에서 DynamoDB 클라이언트에 접근해야 하므로 컨텍스트를 통해 단일 클라이언트 인스턴스를 전달할 수 있도록 해야 합니다. 이를 위해 `packages/game-api/backend/src` 내에서 다음 변경 사항을 적용합니다:
...
마지막 단계는 DynamoDB 테이블을 생성하고 Game API에서 작업 수행 권한을 부여하도록 인프라를 업데이트하는 것입니다. `packages/infra/src`를 다음과 같이 수정합니다:
...
:::note
각 프로시저는 개별 람다 함수에 의해 서비스되므로 최소 권한 원칙을 따라 프로시저 구현에 따라 필요한 읽기/쓰기 권한만 할당할 수 있습니다.
:::
  </TabItem>
</Tabs>
...
인프라 프로덕션과 같은 별도의 스택으로 배포 단계를 분리할 수 있으므로 `--all` 플래그는 원치 않는 배포를 유발할 수 있어 **권장되지 않습니다**!
...
`.bashrc` 파일에 다음 스크립트를 추가하거나(그리고 `source` 실행) 명령어를 실행할 터미널에 직접 붙여넣을 수 있습니다.
```bash
// ~/.bashrc
acurl () {
    REGION=$1
...
그런 다음 sigv4 인증된 curl 요청을 보내려면 다음 예제와 같이 `acurl`을 호출할 수 있습니다:
...
<Tabs>
  <TabItem label="로컬">
    ...
    <Aside type="caution">
    하이라이트된 플레이스홀더를 교체할 때 CDK 배포 출력 값 `dungeon-adventure-infra-sandbox.ElectroDbTableTableNameXXX`을 사용하세요.
    </Aside>
...
```
    <Aside type="caution">
    하이라이트된 플레이스홀더를 교체할 때 CDK 배포 출력 값 `dungeon-adventure-infra-sandbox.GameApiGameApiEndpointXXX`을 사용하고 해당 리전을 설정하세요.
    </Aside>
  </TabItem>
</Tabs>
