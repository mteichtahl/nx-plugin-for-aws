...
tRPC API 생성기는 AWS CDK 인프라 설정이 포함된 새로운 tRPC API를 생성합니다. 생성된 백엔드는 서버리스 배포를 위해 AWS Lambda를 사용하며 [Zod](https://zod.dev/)를 통한 스키마 검증을 포함합니다. 또한 로깅, AWS X-Ray 추적, Cloudwatch 메트릭을 포함한 관측성을 위해 [AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/typescript/latest/)를 설정합니다.
...
위에서 볼 수 있듯이 tRPC API에는 [`schema`](#schema)와 [`backend`](#backend) 두 가지 주요 구성 요소가 있으며, 이들은 작업 공간에서 개별 패키지로 정의됩니다.

:::tip
`schema`와 `backend`는 모두 TypeScript 프로젝트이므로 일반적인 사용법에 대한 자세한 내용은 <Link path="guides/typescript-project">TypeScript 프로젝트 문서</Link>를 참조할 수 있습니다.
:::
...
- `publicProcedure`는 로깅, 추적 및 메트릭을 위한 AWS Lambda Powertools 통합을 포함한 `src/middleware`에 설정된 미들웨어를 포함하여 API의 공개 메서드를 정의합니다.
...
구현 정의에 `query`를 사용하는 것은 작업이 변경을 유발하지 않음을 나타냅니다. 데이터 검색을 위한 메서드를 정의할 때 이 방법을 사용합니다. 변경을 유발하는 작업을 구현하려면 대신 `mutation` 메서드를 사용하세요.
...
구현에서 `TRPCError`를 발생시켜 클라이언트에 오류 응답을 반환할 수 있습니다. 이때 오류 유형을 나타내는 `code`를 지정합니다:
...
AWS Lambda Powertools 메트릭은 `src/middleware/metrics.ts`에 구성되며, API 구현에서 `opts.ctx.metrics`를 통해 접근할 수 있습니다. 이를 사용하여 AWS SDK를 가져오고 사용할 필요 없이 CloudWatch에 메트릭을 기록할 수 있습니다:
...
이 컨텍스트에 추가적인 _선택적_ 속성을 정의합니다. tRPC는 이 미들웨어가 올바르게 구성된 프로시저에서 이 값이 정의되도록 관리합니다.
...
이 경우 Cognito 사용자의 세부 정보를 추출하려고 합니다. REST API 또는 HTTP API에 따라 구현이 약간 다릅니다:
...
tRPC 백엔드 생성기는 `common/constructs` 폴더에 API 배포를 위한 CDK 구문을 생성합니다. 이를 CDK 애플리케이션에서 사용할 수 있습니다:
...
이 설정은 AWS API Gateway REST 또는 HTTP API, 비즈니스 로직을 위한 AWS Lambda 함수, IAM 인증을 포함한 API 인프라를 구성합니다.
...
`grantInvokeAccess` 메서드를 사용하여 API 접근 권한을 부여할 수 있습니다. 예를 들어 인증된 Cognito 사용자에게 API 접근 권한을 부여할 수 있습니다:
...
`serve` 타겟을 사용하여 API의 로컬 서버를 실행할 수 있습니다:
