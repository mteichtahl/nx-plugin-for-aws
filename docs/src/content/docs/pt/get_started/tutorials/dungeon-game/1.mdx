...
- .vscode/
- node_modules/
- packages/ é aqui que seus subprojetos residirão
- .gitignore
- .npmrc
- .prettierignore
- .prettierrc
...
<Aside type="tip">É uma prática recomendada garantir que todos os arquivos não preparados sejam commitados no Git antes de executar quaisquer geradores. Isso permite que você veja o que mudou após executar seu gerador via `git diff`</Aside>
...
Dando uma olhada em alguns dos arquivos-chave:
...
```
O router define o ponto de entrada para sua API tRPC e é o local onde você declarará todos os métodos da API. Como visto acima, temos um método chamado `echo` com sua implementação no arquivo `./procedures/echo.ts`.
...
Este arquivo é a implementação do método `echo` e como visto é fortemente tipado declarando suas estruturas de dados de entrada e saída. Ele está importando essas definições do projeto `:dungeon-adventure/game-api-schema` que é um [alias](https://www.typescriptlang.org/tsconfig/paths.html) para o projeto de schema.

<Aside type="tip">Se você está vendo um erro de importação em seu IDE, isso ocorre porque nosso backend ainda não tem uma referência TypeScript configurada em seu tsconfig.json. O Nx foi [configurado](https://nx.dev/nx-api/js/generators/typescript-sync) para criar essas referências *dinamicamente* sempre que um build/compile é executado ou se você executar o comando `nx sync` manualmente. Para mais informações, consulte o <Link path="guides/typescript-project#importing-your-library-code-in-other-projects">guia TypeScript</Link>.</Aside>
  ...
  constructor(
    ...
    super(scope, id, {
      ...
      policy: new PolicyDocument({
        statements: [
          // Aqui permitimos que qualquer credencial AWS da conta onde o projeto está implantado chame a API.
...
Abaixo está uma lista de todos os arquivos gerados pelo gerador `py#fast-api`. Vamos examinar alguns dos arquivos principais destacados na árvore de arquivos:
<FileTree>
...
- packages/
  - common/
    - constructs/
      ...
      - project.json atualizado para adicionar uma dependência de build em story_api
    - types/ tipos compartilhados
      - src/
...
Aqui está um exemplo do gerador realizando uma transformação AST que preserva todo o código existente e realiza uma atualização. Aqui você pode ver que o `StoryApi` foi adicionado à definição `IRuntimeConfig`, o que significa que quando isso for consumido por nosso frontend, irá impor segurança de tipos!
...
Este é onde todos os seus métodos de API serão definidos. Como visto aqui, temos um método `read_root` mapeado para a rota `GET /`. Você pode usar [Pydantic](https://docs.pydantic.dev/latest/) para declarar entradas e saídas de métodos e garantir segurança de tipos.

</Drawer>

### Interface do Jogo: Website

Agora vamos criar a UI que permitirá que você interaja com o jogo. Para isso, vamos criar um website chamado `GameUI` seguindo os passos abaixo:

<RunGenerator generator="ts#cloudscape-website" requiredParameters={{name:"GameUI"}} noInteractive />

Você deve ver alguns novos arquivos aparecerem em sua árvore de arquivos.

<Drawer title="Arquivos atualizados pelo ts#cloudscape-website" trigger="Clique aqui para examinar esses arquivos em mais detalhes.">
Abaixo está uma lista de todos os arquivos gerados pelo gerador `ts#cloudscape-website`. Vamos examinar alguns dos arquivos principais destacados na árvore de arquivos:
<FileTree>
- packages/
  - common/
    - constructs/
...
Este é o ponto de entrada onde o React é montado. Como mostrado, ele inicialmente apenas configura um `@tanstack/react-router` em uma configuração de [`roteamento baseado em arquivo`](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing). Isso significa que, enquanto seu servidor de desenvolvimento estiver em execução - você pode simplesmente criar arquivos na pasta `routes` e o `@tanstack/react-router` criará a configuração de boilerplate para você, atualizando o arquivo `routeTree.gen.ts`. Este arquivo mantém todas as rotas de forma type-safe, o que significa que ao usar `<Link>`- a opção `to` só mostrará rotas válidas. Para mais informações, consulte a [documentação do `@tanstack/react-router`](https://tanstack.com/router/v1/docs/framework/react/quick-start).
...
Você deve ver alguns arquivos novos/modificados em sua árvore de arquivos.

<Drawer title="Arquivos atualizados pelo ts#cloudscape-website#auth" trigger="Clique aqui para examinar esses arquivos em mais detalhes.">
Abaixo está uma lista de todos os arquivos gerados/atualizados pelo gerador `ts#cloudscape-website#auth`. Vamos examinar alguns dos arquivos principais destacados na árvore de arquivos:
<FileTree>
- packages/
  - common/
    - constructs/
      - src/
        - core/
          - user-identity.ts construct CDK para criar user/identity pools
    - types/
      - src/
  ...
  - game-ui/
    - src/
      - components/
        ...
        - RuntimeConfig/
          - index.tsx busca o `runtime-config.json` e fornece via contexto
      - hooks/
...
### Interface do Jogo: Conectar à Story API

Agora vamos configurar nossa Game UI para se conectar à nossa Story API previamente criada:
...
Abaixo está uma lista de todos os arquivos gerados/atualizados pelo gerador `api-connection`. Vamos examinar alguns dos arquivos principais destacados na árvore de arquivos:
<FileTree>
- packages/
  - game-ui/
    - src/
...
### Interface do Jogo: Conectar à Game API

Agora vamos configurar nossa Game UI para se conectar à nossa Game API previamente criada:
...
<Aside>
O hook `useGameApi` é diferente do `useStoryApi` pois não requer um build para que mudanças sejam refletidas, graças ao uso de [inferência TypeScript](https://trpc.io/docs/concepts) pelo tRPC. Isso permite que desenvolvedores façam alterações no backend que são instantaneamente refletidas no frontend!
</Aside>
...
### Interface do Jogo: Infraestrutura
...
Abaixo está uma lista de todos os arquivos gerados/atualizados pelo gerador `ts#infra`. Vamos examinar alguns dos arquivos principais destacados na árvore de arquivos:
<FileTree>
- packages/
  - common/
    - constructs/
...
Este é onde vamos instanciar nossos constructs CDK para construir nosso jogo Dungeon Adventure.
...
+    );
+
+    // Garanta que isto seja instanciado por último para que o runtime-config.json seja configurado automaticamente
...
Você também pode disparar um build (ou qualquer outra tarefa) para um target de projeto único executando o target no projeto diretamente. Por exemplo, se quisermos construir o projeto `@dungeon-adventure/infra`, você pode executar:
...
O Nx depende de [cache](https://nx.dev/concepts/how-caching-works) para que você possa reutilizar artefatos de builds anteriores e acelerar o desenvolvimento. Há alguma configuração necessária para que isso funcione corretamente e pode haver casos onde você queira realizar um build **sem usar o cache**. Para isso, simplesmente adicione o argumento `--skip-nx-cache` ao seu comando. Por exemplo:
...
Esta mensagem indica que o NX detectou alguns arquivos que podem ser atualizados automaticamente. Neste caso, refere-se aos arquivos `tsconfig.json` que não possuem referências TypeScript configuradas para projetos dependentes. Selecione a opção **Yes, sync the changes and run the tasks** para prosseguir. Você deve notar que todos os erros de importação relacionados ao IDE são resolvidos automaticamente, pois o gerador de sincronização adicionará as referências TypeScript faltantes automaticamente!
...
Se encontrar um erro de build/synth para o projeto `@dungeon-adventure/infra`, isso é esperado pois a biblioteca que instrumenta o `cfn-guard` atualmente não suporta Windows. Há uma solicitação de feature acompanhando isso, mas por enquanto podemos simplesmente desabilitar o `cfn-guard` modificando o arquivo `packages/infra/src/main.ts` da seguinte forma:
...
Todos os artefatos construídos estão agora disponíveis na pasta `dist/` localizada na raiz do monorepo. Esta é uma prática padrão ao usar projetos gerados pelo `@aws/nx-plugin` pois não polui sua árvore de arquivos com arquivos gerados. Caso queira limpar seus arquivos, você pode simplesmente deletar a pasta `dist/` sem se preocupar com arquivos gerados espalhados pela árvore de arquivos.

Parabéns! Você criou todos os subprojetos necessários para começar a implementar o núcleo do nosso jogo Dungeon Adventure.  🎉🎉🎉