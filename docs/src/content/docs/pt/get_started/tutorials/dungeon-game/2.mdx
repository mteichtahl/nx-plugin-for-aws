...
Vamos implementar nosso banco de dados no DynamoDB usando a biblioteca cliente [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/) para simplificar o processo. Para começar, primeiro precisamos instalar o `electrodb` executando o seguinte comando:
...
Agora vamos criar os seguintes arquivos em nossa pasta `packages/game-api/src/entities` para definir nossas entidades ElectroDB de acordo com o diagrama ER acima:
...
O passo final é atualizar nossa infraestrutura para criar a tabela DynamoDB e conceder permissões para operações na Game API. Para isso, atualize o `packages/infra/src` conforme abaixo:
...
:::note
Observe que como cada procedimento é atendido por uma função lambda individual, podemos seguir o princípio do menor privilégio e atribuir apenas as permissões de leitura/escrita necessárias com base na implementação do procedimento.
:::
  </TabItem>
</Tabs>
...
:::tip
Se estiver iterando em alterações de código das funções lambda, você pode implantar com a flag `--hotswap` após construir a base de código para um tempo de implantação muito menor (2-3 segundos).
...
Você pode adicionar o seguinte script ao seu arquivo `.bashrc` (e executar `source`) ou colar diretamente no terminal onde executará os comandos.
```bash
// ~/.bashrc
acurl () {
    REGION=$1
...
Adicione a função ao seu perfil PowerShell ou cole diretamente na sessão atual.
```powershell
function acurl {
    param(
...
<Tabs>
  <TabItem label="Local">
    ...
    Com o servidor em execução, teste com:
    ```bash
...
:::note
O `%7B%7D` passado no teste é um objeto JSON vazio (`{}`) codificado em URI.
:::

Se o comando for bem-sucedido, você verá a resposta:
```json
