...
<Aside>
Todas as dependências são adicionadas ao `package.json` raiz pois o `@aws/nx-plugin` segue o princípio de [política de versão única](https://nx.dev/concepts/decisions/dependency-management#single-version-policy). Para mais detalhes, consulte o <Link path="guides/typescript-project#dependencies">guia ts#project</Link>.
</Aside>
...
O ElectroDB é poderoso e nos permite não apenas definir tipos, mas também fornecer valores padrão (como os timestamps acima). Além disso, segue o [design de tabela única](https://electrodb.dev/en/core-concepts/single-table-relationships/), considerada melhor prática no DynamoDB.
...
Como precisamos do cliente DynamoDB em nossos procedimentos, vamos criar uma instância única e passá-la via contexto. Faça as seguintes alterações em `packages/game-api/backend/src`:

<Tabs>
  <TabItem label="middleware/dynamodb.ts">
<E2ECode lang="typescript" path="dungeon-adventure/2/middleware/dynamodb.ts.template" />

Este é um plugin que criamos para injetar o `DynamoDBClient` no contexto.
  </TabItem>
...
Estendemos o `IMiddlewareContext` para adicionar `IDynamoDBContext`.
  </TabItem>
...
O plugin DynamoDB é configurado.
...
Vamos implementar os métodos da API. Faça as seguintes alterações em `packages/game-api/backend/src/procedures`:
...
Agora vamos conectar nossos procedimentos à API. Atualize o arquivo:

<E2EDiff lang="typescript" before="dungeon-adventure/2/router.ts.old.template" after="dungeon-adventure/2/router.ts.template" />

### Infraestrutura

A etapa final é atualizar a infraestrutura para criar a tabela DynamoDB e conceder permissões. Atualize `packages/infra/src`:
...
:::note
Como cada procedimento é atendido por uma função lambda separada, podemos seguir o princípio de menor privilégio e atribuir apenas as permissões necessárias.
:::
  </TabItem>
</Tabs>
...
A primeira implantação leva ~8 minutos. As subsequentes levam ~2 minutos.

:::tip
Para alterações em lambdas, use `--hotswap` após construir para implantações rápidas (~3s):
...
<Tabs>
  <TabItem label="Local">
    ...
    <Aside type="caution">
    Use o valor `dungeon-adventure-infra-sandbox.ElectroDbTableTableNameXXX` do output do CDK.
    </Aside>
...
```
    <Aside type="caution">
    Use o endpoint `dungeon-adventure-infra-sandbox.GameApiGameApiEndpointXXX` do output do CDK.
    </Aside>
  </TabItem>
</Tabs>

:::note
O parâmetro `%7B%7D` é um objeto JSON vazio (`{}`) codificado.
:::
...
Parabéns, você implementou sua primeira API com tRPC! 🎉🎉🎉