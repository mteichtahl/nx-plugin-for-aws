...
O construct CDK gerado em `common/constructs` pode ser usado em aplicações CDK:
...
Para consumir a API de um site React, use o gerador <Link path="guides/api-connection/react-fastapi">`api-connection`</Link>.