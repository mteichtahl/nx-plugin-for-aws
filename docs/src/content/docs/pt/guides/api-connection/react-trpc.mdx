...
4. Cognito Auth adicionado via <Link path="/guides/cloudscape-website-auth">gerador `ts#cloudscape-website-auth`</Link> se conectando a uma API que usa autenticação Cognito ou IAM
...
- src
  ...
  - hooks
    ...
    - use\<ApiName>.tsx Hook para a API de backend específica. ApiName será resolvido para o nome da API
...
Os tipos são inferidos automaticamente das definições do router e schema do backend, garantindo que quaisquer mudanças na API sejam imediatamente refletidas no código frontend sem necessidade de build.
