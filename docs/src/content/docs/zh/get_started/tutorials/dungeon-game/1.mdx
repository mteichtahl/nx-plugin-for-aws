...
- package.json 所有 node 依赖项定义在此
- pnpm-lock.yaml 或 bun.lock, yarn.lock, package-lock.json (根据包管理器不同)
...
<Aside type="tip">最佳实践建议在运行任何生成器之前提交所有未暂存的文件到 Git。这允许您通过 `git diff` 查看生成器运行后的变化</Aside>
...
<Aside>
根目录的 `package.json` 现在配置了 `"type": "module"`，这意味着由 `@aws/nx-plugin` 生成的基于 node 的子项目默认使用 ESM 模块类型。有关 TypeScript 项目的更多细节，请参考 <Link path="guides/typescript-project">ts#project 指南</Link>。
</Aside>
...
该文件是 `echo` 方法的实现，通过声明输入输出数据结构实现强类型。这些定义从 `:dungeon-adventure/game-api-schema` 项目导入，这是 schema 项目的 [路径别名](https://www.typescriptlang.org/tsconfig/paths.html)。

<Aside type="tip">如果在 IDE 中看到导入错误，这是因为后端尚未在其 tsconfig.json 中设置 typescript 引用。Nx 已配置为在运行构建/编译时或手动运行 `nx sync` 命令时[动态创建](https://nx.dev/nx-api/js/generators/typescript-sync)这些引用。更多信息请参考 <Link path="guides/typescript-project#importing-your-library-code-in-other-projects">Typescript 指南</Link>。</Aside>
...
/**
 * 用于创建和配置 AWS API Gateway REST API 的 CDK 构造
 * @template TIntegrations - 操作名称到集成的映射
 */
...
这是定义 GameApi 的 CDK 构造。其中的 `defaultIntegrations` 方法会自动为 tRPC API 中的每个过程创建 lambda 函数，指向已打包的 API 实现。这意味着在 `cdk synth` 时不会进行打包操作（与使用 [NodeJsFunction](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-cdk-lib.aws_lambda_nodejs.NodejsFunction.html) 不同），因为我们已经通过后端项目的构建目标完成了打包。
...
这是定义 StoryApi 的 CDK 构造。其中的 `defaultIntegrations` 方法会自动为 FastAPI 中的每个操作创建 lambda 函数，指向已打包的 API 实现。这意味着在 `cdk synth` 时不会进行打包操作（与使用 [PythonFunction](https://docs.aws.amazon.com/cdk/api/v2/docs/@aws-cdk_aws-lambda-python-alpha.PythonFunction.html) 不同），因为我们已经通过后端项目的构建目标完成了打包。
...
这是生成器执行 AST 转换的示例，保留现有代码并执行更新。此处可见 `StoryApi` 被添加到 `IRuntimeConfig` 定义中，这意味着当最终前端消费此配置时，将强制执行类型安全！
...
这是定义所有 API 方法的位置。如上所示，我们有一个映射到 `GET /` 路由的 `read_root` 方法。您可以使用 [Pydantic](https://docs.pydantic.dev/latest/) 声明方法输入和输出以确保类型安全。
...
- packages/
  ...
  - game-ui/
    ...
    - src/
      ...
      - routes/ @tanstack/react-router 文件式路由
...
这是定义 GameUI 的 CDK 构造。其已配置为指向基于 Vite 的 UI 生成包。这意味着在构建时，打包操作在 game-ui 项目的构建目标中完成，其输出在此处使用。
...
这是挂载 React 的入口点。如上所示，初始配置使用 [`文件式路由`](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing) 的 `@tanstack/react-router`。只要开发服务器在运行，您只需在 `routes` 文件夹中创建文件，`@tanstack/react-router` 就会自动生成样板文件并更新 `routeTree.gen.ts` 文件。该文件以类型安全的方式维护所有路由，因此当使用 `<Link>` 时，`to` 选项仅显示有效路由。更多信息请参考 [`@tanstack/react-router` 文档](https://tanstack.com/router/v1/docs/framework/react/quick-start)。
...
function RouteComponent() {
  return (
    <ContentLayout header={<Header>Welcome</Header>}>
      <SpaceBetween size="l">
        <Container>欢迎使用您的新 Cloudscape 网站！</Container>
      </SpaceBetween>
    </ContentLayout>
  );
}
...
该组件将在导航到 `/welcome` 路由时渲染。只要开发服务器在运行，`@tanstack/react-router` 会在创建/移动此文件时自动管理 `Route`。这将在本教程后续章节中展示。
...
通过 AST 转换将 `RuntimeConfigProvider` 和 `CognitoAuth` 组件添加到 `main.tsx` 文件。这使得 `CognitoAuth` 组件可以通过获取包含 Cognito 连接配置的 `runtime-config.json` 来与 Amazon Cognito 进行认证，确保后端调用指向正确目标。

</Drawer>

### 游戏 UI：连接故事 API

现在配置 Game UI 连接到之前创建的 Story API：
...
该钩子可用于向 `StoryApi` 发起认证 API 请求。如实现所示，它使用构建时生成的 `StoryApi`，因此在构建代码前 IDE 中会显示错误。有关客户端生成或如何消费 API 的更多细节，请参考 <Link path="guides/api-connection/react-fastapi">React 到 FastAPI 指南</Link>。
...
上述提供者组件使用 `useStoryApiClient` 钩子，并实例化 `StoryApiOptionsProxy`，用于构建 TanStack Query 钩子的选项。您可以使用相应的钩子 `useStoryApi` 访问此选项代理，以与 tRPC API 一致的方式与 FastAPI 交互。

由于 `useStoryApiClient` 为我们的流式 API 提供了异步迭代器，在本教程中我们将直接使用原生客户端。

<Aside type="caution">
`src/generated/story-api/*.gen.ts` 文件永远不应手动修改，因为它们会在每次构建 API 时重新生成。
</Aside>
...
现在配置 Game UI 连接到之前创建的 Game API：
...
该钩子使用 tRPC 最新的 [React Query 集成](https://trpc.io/blog/introducing-tanstack-react-query-client)，允许用户直接与 `@tanstack/react-query` 交互而无需额外抽象层。有关调用 tRPC API 的示例，请参考 <Link path="guides/api-connection/react-trpc#using-the-generated-code">使用 tRPC 钩子指南</Link>。

<Aside>
`useGameApi` 钩子与 `useStoryApi` 不同，它不需要构建即可反映更改，这得益于 tRPC 的 [Typescript 类型推断](https://trpc.io/docs/concepts)。这使得开发者对后端所做的更改能即时反映在前端！
</Aside>
...
- packages/
  ...
  - infra
    - src/
      - stacks/
        - **application-stack.ts** CDK 资源定义在此
      - index.ts
      - **main.ts** 定义所有堆栈的入口点
    - cdk.json
    - project.json
...
配置为使用 [`cfn-guard`](https://github.com/cdklabs/cdk-validator-cfnguard) 根据配置的规则集进行基础设施验证。该验证在合成后执行。

<Aside type="tip">
有时可能需要抑制某些资源上的规则。有两种方式实现：
...
...
// 如果构造是 Bucket 实例，则抑制其及其子构造的 RULE_NAME
...
注意此处我们为两个 API 提供了默认集成。默认情况下，API 中的每个操作都映射到独立的 lambda 函数来处理。
...
`run-many` 命令会在多个列出的子项目上运行目标（`--all` 将针对所有项目）。它会确保依赖项按正确顺序执行。

您也可以通过直接在项目上运行目标来触发单个项目的构建（或其他任务）。例如，要构建 `@dungeon-adventure/infra` 项目：
...
Nx 依赖[缓存](https://nx.dev/concepts/how-caching-works)以便重用先前构建产物加速开发。需要一些配置才能正常工作，有时可能需要执行**不使用缓存**的构建。为此，只需在命令后添加 `--skip-nx-cache` 参数。例如：

<NxCommands commands={['run @dungeon-adventure/infra:build --skip-nx-cache']} />
如果需要清除缓存（存储在 `.nx` 文件夹），可运行：
...
此消息表示 NX 检测到可自动更新的文件。此处指的是缺少对依赖项目的 Typescript 引用的 `tsconfig.json` 文件。选择 **Yes, sync the changes and run the tasks** 继续。您会注意到所有 IDE 导入错误自动解决，因为同步生成器会自动添加缺失的 Typescript 引用！
...
<Drawer trigger="如果在 Windows 上遇到构建错误，点击此处。" title="Windows 构建失败">
如果 `@dungeon-adventure/infra` 项目构建/合成失败，这是预期的，因为当前用于 `cfn-guard` 的库不支持 Windows。已有功能请求跟踪此问题，在此期间我们可以通过修改 `packages/infra/src/main.ts` 文件禁用 `cfn-guard`：
...
恭喜！您已成功创建了实现地牢冒险游戏核心功能所需的所有子项目。🎉🎉🎉