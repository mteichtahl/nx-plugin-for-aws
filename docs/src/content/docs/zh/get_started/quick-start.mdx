...
本快速入门将添加 tRPC API、React 网站、Cognito 身份验证和 CDK 基础设施。根据项目类型，您可以选择任意组合的生成器快速初始化项目。查看左侧导航栏的 **指南** 获取完整选项列表。

### 添加 tRPC API

<RunGenerator generator="ts#trpc-api" requiredParameters={{ apiName: 'demo-api' }} />

这将在 `packages/demo-api` 目录创建 API。

### 添加 React 网站

<RunGenerator generator="ts#cloudscape-website" requiredParameters={{ name: 'demo-website' }} />

这将在 `packages/demo-website` 创建新的 React 网站。
...
恭喜！🎉 您已成功使用 `@aws/nx-plugin` 构建并部署了完整应用！