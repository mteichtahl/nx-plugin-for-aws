...
tRPC API 生成器会创建一个配置了 AWS CDK 基础设施的新 tRPC API。生成的后端使用 AWS Lambda 进行无服务器部署，并通过 [Zod](https://zod.dev/) 实现模式验证。它集成了 [AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/typescript/latest/) 用于可观测性，包括日志记录、AWS X-Ray 追踪和 Cloudwatch 指标。
...
如上所示，tRPC API 包含两个主要组件：[`schema`](#schema) 和 [`backend`](#backend)，在您的工作区中定义为独立包。
...
模式由服务端和客户端代码共享，在修改 API 使用的数据结构时只需在单一位置更新。
...
嵌套的 `backend` 文件夹包含 API 实现，您需要在此定义 API 操作及其输入、输出和实现。

API 入口点位于 `src/router.ts`。该文件包含 Lambda 处理程序，根据调用的操作将请求路由到对应"过程"。每个过程定义预期的输入、输出和实现。
...
- `publicProcedure` 定义 API 的公共方法，包含 `src/middleware` 中设置的中间件。这些中间件包含用于日志、追踪和指标的 AWS Lambda Powertools 集成。
- `input` 接受 Zod 模式，定义操作的预期输入。发送到此操作的请求会自动根据该模式验证。
- `output` 接受 Zod 模式，定义操作的预期输出。如果实现返回不符合模式的输出，将出现类型错误。
- `query` 接受定义操作实现的函数。该实现接收 `opts` 参数，包含传递给操作的 `input`，以及中间件设置的上下文（通过 `opts.ctx` 访问）。传递给 `query` 的函数必须返回符合 `output` 模式的输出。

使用 `query` 定义实现表示该操作是非变更性的。用于定义数据检索方法。要实现变更性操作，请改用 `mutation` 方法。

添加新操作时，请确保在 `src/router.ts` 的路由中注册。
...
客户端将接收此操作分组，例如调用上述 `listUsers` 操作可能如下所示：
...
AWS Lambda Powertools 日志记录器配置于 `src/middleware/logger.ts`，可通过 `opts.ctx.logger` 在 API 实现中访问。可用于记录到 CloudWatch Logs 或控制每个结构化日志消息包含的额外值。例如：
...
有关日志记录的更多信息，请参阅 [AWS Lambda Powertools 日志记录文档](https://docs.powertools.aws.dev/lambda/typescript/latest/core/logger/)。

### 记录指标

AWS Lambda Powertools 指标配置于 `src/middleware/metrics.ts`，可通过 `opts.ctx.metrics` 在 API 实现中访问。可用于无需导入 AWS SDK 即可在 CloudWatch 中记录指标，例如：
...
AWS Lambda Powertools 追踪器配置于 `src/middleware/tracer.ts`，可通过 `opts.ctx.tracer` 在 API 实现中访问。可用于通过 AWS X-Ray 添加追踪以深入了解 API 请求的性能和流程。例如：
...
例如，在 `src/middleware/identity.ts` 中实现中间件来从 API 提取调用用户详细信息。
...
注意我们向上下文定义了一个额外的_可选_属性。tRPC 会管理确保在正确配置此中间件的过程中有此定义。
...
在本例中，我们需要提取调用 Cognito 用户的详细信息。通过从 API Gateway 事件中提取用户主题 ID（"sub"）并从 Cognito 检索用户详细信息实现。具体实现根据事件是由 REST API 还是 HTTP API 提供而有所不同：
...
tRPC 后端生成器在 `common/constructs` 文件夹中生成用于部署 API 的 CDK 构造。可在 CDK 应用程序中使用，例如：
...
这将设置 API 基础设施，包括 AWS API Gateway REST/HTTP API、业务逻辑的 AWS Lambda 函数和 IAM 认证。
...
可创建 tRPC 客户端以类型安全的方式调用 API。若从其他后端调用 tRPC API，可使用 `src/client/index.ts` 中的客户端，例如：
...
若从 React 网站调用 API，请考虑使用 <Link path="guides/api-connection/react-trpc">API 连接</Link> 生成器来配置客户端。
