...
FastAPI 生成器会创建一个带有 AWS CDK 基础设施配置的新 FastAPI 项目。生成的后端使用 AWS Lambda 进行无服务器部署，并通过 AWS API Gateway API 对外暴露。项目集成了 [AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/python/latest/) 用于可观测性，包括日志记录、AWS X-Ray 追踪和 Cloudwatch 指标。
...
可使用 <Link path="/guides/cloudscape-website-auth">`ts#cloudscape-website-auth` 生成器</Link> 生成 `UserIdentity` 构造
:::
...
:::note
当 API 发生变更时，需要运行构建以确保 CDK 构造使用的类型保持最新。
