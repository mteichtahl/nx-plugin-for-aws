...
:::note
Se agrega un alias en `tsconfig.base.json` en la raíz de tu monorepo para resolver Zod a [versión 4.x](https://zod.dev/v4). Esto prepara para cuando Zod v4 sea lanzado en NPM, permitiendo una actualización sin cambiar rutas de importación.
:::
...
Los clientes verán esta agrupación, por ejemplo invocar la operación `listUsers` se vería así:
...
El logger de AWS Lambda Powertools se configura en `src/middleware/logger.ts`, y se puede acceder en una implementación de API via `opts.ctx.logger`. Puedes usarlo para registrar en CloudWatch Logs, y/o controlar valores adicionales a incluir en cada mensaje de log estructurado. Por ejemplo:
...
Las métricas de AWS Lambda Powertools se configuran en `src/middleware/metrics.ts`, y se puede acceder a ellas via `opts.ctx.metrics`. Puedes usarlas para registrar métricas en CloudWatch sin necesidad de usar el SDK de AWS, por ejemplo:
...
El tracer de AWS Lambda Powertools se configura en `src/middleware/tracer.ts`, y se puede acceder via `opts.ctx.tracer`. Puedes usarlo para agregar trazas con AWS X-Ray y obtener insights detallados sobre el rendimiento y flujo de las solicitudes. Por ejemplo:
...
:::warning
Este ejemplo asume que `auth` se configuró como `IAM`. Para autenticación Cognito, el middleware de identidad es más directo, extrayendo los claims relevantes del `event`.
:::
...
Nota que definimos una propiedad _opcional_ adicional en el contexto. tRPC se encarga de asegurar que esto esté definido en procedimientos que tengan correctamente configurado este middleware.
...
En nuestro caso, queremos extraer detalles del usuario Cognito que llama. Hacemos esto extrayendo el ID de sujeto (o "sub") del usuario del evento de API Gateway, y recuperando detalles del usuario de Cognito. La implementación varía ligeramente dependiendo de si el evento fue proporcionado por una REST API o HTTP API:
...
Esto configura la infraestructura de tu API, incluyendo AWS API Gateway REST o HTTP API, funciones AWS Lambda para lógica de negocio, y autenticación basada en tu método `auth` seleccionado.
...
Si seleccionaste autenticación `IAM`, puedes usar el método `grantInvokeAccess` para conceder acceso a tu API. Por ejemplo, podrías querer dar acceso a usuarios autenticados de Cognito:
