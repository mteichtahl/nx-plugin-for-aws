...
:::consejo
`schema` y `backend` son ambos proyectos TypeScript, por lo que puedes consultar la <Link path="guides/typescript-project">documentación de proyectos TypeScript</Link> para más detalles sobre su uso general.
:::
...
El tracer de AWS Lambda Powertools se configura en `src/middleware/tracer.ts`, y puede accederse en una implementación de API via `opts.ctx.tracer`. Puedes usarlo para añadir trazas con AWS X-Ray y obtener insights detallados sobre el rendimiento y flujo de las solicitudes API. Por ejemplo:
...
El generador de backend tRPC genera un construct CDK para desplegar tu API en la carpeta `common/constructs`. Puedes consumirlo en una aplicación CDK, por ejemplo:
...
Esto configura la infraestructura de tu API, incluyendo una API REST o HTTP de AWS API Gateway, funciones AWS Lambda para la lógica de negocio, y autenticación basada en tu método `auth` elegido.
...
Si seleccionaste usar autenticación `IAM`, puedes usar el método `grantInvokeAccess` para conceder acceso a tu API, por ejemplo podrías querer dar acceso a usuarios autenticados de Cognito:
...
Si estás llamando a tu API desde un sitio web React, considera usar el generador <Link path="guides/api-connection/react-trpc">API Connection</Link> para configurar el cliente.
