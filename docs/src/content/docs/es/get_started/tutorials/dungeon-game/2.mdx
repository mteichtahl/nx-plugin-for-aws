...
<Aside type="tip">
Como se muestra arriba, para cada esquema definido en Zod también exportamos una interfaz usando la sintaxis `z.TypeOf`. ¡Esto convierte nuestra definición Zod en una interfaz TypeScript sin duplicar esfuerzos!
</Aside>
...
Como necesitamos acceso al cliente DynamoDB en cada procedimiento, crearemos una instancia única que pasaremos mediante el contexto. Para esto, realiza los siguientes cambios en `packages/game-api/backend/src`:
...
Implementemos ahora los métodos de la API. Realiza los siguientes cambios en `packages/game-api/backend/src/procedures`:
...
El paso final es actualizar nuestra infraestructura para crear la tabla DynamoDB y otorgar permisos. Actualiza `packages/infra/src`:
...
:::note
Como cada procedimiento se sirve mediante una función lambda independiente, podemos aplicar el principio de mínimo privilegio asignando solo los permisos necesarios.
:::
  </TabItem>
</Tabs>
...
:::tip
Para cambios en código lambda, usa el flag `--hotswap` después de construir para despliegues rápidos (2-3 segundos):
...
**No recomendado** si tienes stacks separados (ej: infra-prod), ya que desplegaría todo indiscriminadamente.
...
Añade este script a tu `.bashrc` (y ejecuta `source`) o pégalo en la terminal:
...
<Tabs>
  <TabItem label="Local">
    ...
    <Aside type="caution">
    Usa el valor `dungeon-adventure-infra-sandbox.ElectroDbTableTableNameXXX` de la salida CDK para reemplazar el marcador.
    </Aside>
...
¡Felicidades, has implementado y desplegado tu primera API con tRPC! 🎉🎉🎉