...
Comenzaremos creando un nuevo monorepo. Desde el directorio de tu elección, ejecuta el siguiente comando:

<CreateNxWorkspaceCommand workspace="dungeon-adventure" />

Esto configurará un monorepo NX dentro del directorio `dungeon-adventure`, que luego podrás abrir en vscode. Debería verse así:
...
<Aside type="tip">Es una buena práctica asegurarse de que todos tus archivos no preparados estén confirmados en Git antes de ejecutar cualquier generador. Esto te permite ver qué ha cambiado después de ejecutar tu generador mediante `git diff`</Aside>
...
<Drawer title="Archivos actualizados por ts#trpc-api" trigger="Haz clic aquí para examinar estos archivos con más detalle.">
...
```
El router define el punto de entrada para tu API tRPC y es el lugar donde declararás todos los métodos de tu API. Como puedes ver arriba, tenemos un método llamado `echo` con su implementación en el archivo `./procedures/echo.ts`.
  ...
  constructor(
    ...
    super(scope, id, {
      ...
      policy: new PolicyDocument({
        statements: [
          // Aquí permitimos que cualquier credencial AWS de la cuenta en la que se despliega el proyecto pueda llamar a la API.
...
Deberías ver que han aparecido algunos archivos nuevos en tu árbol de archivos.
<Drawer title="Archivos actualizados por py#fast-api" trigger="Haz clic aquí para examinar estos archivos con más detalle.">
...
Aquí hay un ejemplo del generador realizando una transformación AST que preserva todo el código existente y realiza una actualización. Puedes ver que se agregó `StoryApi` a la definición de `IRuntimeConfig`, lo que significa que cuando esto sea consumido por nuestro frontend, ¡impondrá seguridad de tipos!
...
<Drawer title="Archivos actualizados por ts#cloudscape-website" trigger="Haz clic aquí para examinar estos archivos con más detalle.">
...
Este es el construct CDK que define nuestro GameUI. Como puedes ver, ya ha configurado la ruta al paquete generado para nuestra UI basada en Vite. Esto significa que en el momento de `build`, el empaquetado ocurre dentro del objetivo de compilación del proyecto game-ui y su salida se usa aquí.
...
<Drawer title="Archivos actualizados por ts#cloudscape-website#auth" trigger="Haz clic aquí para examinar estos archivos con más detalle.">
...
<Drawer title="Archivos actualizados por la conexión UI -> FastAPI" trigger="Haz clic aquí para examinar estos archivos con más detalle.">
...
- packages/
  - game-ui/
    ...
    - project.json actualizado para agregar objetivos para generar hooks openapi
    - ...
  - story_api/
    - scripts/
...
Dado que `useStoryApiClient` nos proporciona un iterador asíncrono para nuestra API de streaming, en este tutorial simplemente usaremos el cliente vanilla directamente.

<Aside type="caution">
Los archivos `src/generated/story-api/*.gen.ts` nunca deben modificarse manualmente, ya que se regenerarán cada vez que compiles tu API.
</Aside>
...
<Drawer title="Archivos actualizados por la conexión UI -> tRPC" trigger="Haz clic aquí para examinar estos archivos con más detalle.">
A continuación se muestra una lista de todos los archivos generados/actualizados por el generador `api-connection`. Vamos a examinar algunos de los archivos clave resaltados en el árbol de archivos:
<FileTree>
- packages/
  - game-ui/
...
<Drawer title="Archivos actualizados por ts#infra" trigger="Haz clic aquí para examinar estos archivos con más detalle.">
...
Está configurado para usar [`cfn-guard`](https://github.com/cdklabs/cdk-validator-cfnguard) y ejecutar validaciones de infraestructura basadas en el conjunto de reglas configurado. Esto se instrumenta post síntesis.
...
Observa aquí que proporcionamos integraciones predeterminadas para nuestras dos APIs. Por defecto, cada operación en nuestra API se mapea a una función lambda individual para manejar esa operación.
...
También puedes desencadenar una compilación (o cualquier otra tarea) para un objetivo de proyecto único ejecutando el objetivo directamente en el proyecto. Por ejemplo, si queremos compilar el proyecto `@dungeon-adventure/infra`, puedes ejecutar el siguiente comando:
...
Nx depende del [caché](https://nx.dev/concepts/how-caching-works) para que puedas reutilizar artefactos de compilaciones anteriores y acelerar el desarrollo. Se requiere cierta configuración para que esto funcione correctamente y puede haber casos donde quieras realizar una compilación **sin usar el caché**. Para eso, simplemente agrega el argumento `--skip-nx-cache` a tu comando. Por ejemplo:

<NxCommands commands={['run @dungeon-adventure/infra:build --skip-nx-cache']} />
Si por alguna razón quisieras borrar tu caché (almacenado en la carpeta `.nx`), puedes ejecutar el siguiente comando:
