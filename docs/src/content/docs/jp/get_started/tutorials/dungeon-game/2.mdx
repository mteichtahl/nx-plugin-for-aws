...
このプロジェクトで使用しない`packages/game-api/src/schema/echo.ts`ファイルは削除してください。

<Aside type="tip">
上記の通り、Zodで定義した各スキーマに対して`z.TypeOf`構文を使用してインターフェースをエクスポートしています。これにより、Zod定義をTypescriptインターフェースに変換でき、重複作業を回避できます！
</Aside>
...
DynamoDBにデータベースを実装し、[ElectroDB](https://electrodb.dev/en/core-concepts/introduction/)クライアントライブラリを使用します。まず以下のコマンドで`electrodb`をインストールします：

<InstallCommand pkg="electrodb @aws-sdk/client-dynamodb" />

<Aside>
全ての依存関係はルートの`package.json`に追加されます。これは`@aws/nx-plugin`が[シングルバージョンポリシー](https://nx.dev/concepts/decisions/dependency-management#single-version-policy)に従っているためです。詳細は<Link path="guides/typescript-project#dependencies">tsプロジェクトガイド</Link>を参照してください。
</Aside>

`packages/game-api/src/entities`フォルダ内に以下のファイルを作成し、上記ER図に基づいてElectroDBエンティティを定義します：
...
ElectroDBは非常に強力で、タイプ定義だけでなくタイムスタンプなどのデフォルト値も設定できます。またDynamoDBのベストプラクティスである[シングルテーブル設計](https://electrodb.dev/en/core-concepts/single-table-relationships/)を採用しています。
...
各プロシージャでDynamoDBクライアントにアクセスするため、コンテキスト経由でクライアントインスタンスを渡せるようにします。`packages/game-api/src`内で以下の変更を加えます：
...
DynamoDBプラグインを初期化します。

<Aside>
`concat` APIはミドルウェアをプロシージャにバインドします。詳細は[concatガイド](https://trpc.io/docs/server/middlewares#concat)を参照してください。
</Aside>
  </TabItem>
</Tabs>
...
使用しない`echo.ts`ファイル（`packages/game-api/src/procedures`内）は削除してください。
...
最後にDynamoDBテーブルを作成し、Game APIに操作権限を付与するため、`packages/infra/src`を更新します：
...
:::note
各プロシージャは個別のLambda関数で実行されるため、最小権限の原則に従い、プロシージャの実装に基づいて必要な読み書き権限のみを割り当てます。
:::
  </TabItem>
</Tabs>
...
:::tip
Lambda関数のコード変更を繰り返す場合、`--hotswap`フラグを使用してデプロイすると（コードビルド後）、デプロイ時間が大幅に短縮されます（2-3秒）。
...
<Tabs>
  <TabItem label="ローカル環境">
    ...
    <Aside type="caution">
    CDKデプロイ出力の`dungeon-adventure-infra-sandbox.ElectroDbTableTableNameXXX`値でプレースホルダーを置き換えてください。
    </Aside>
...
```
    <Aside type="caution">
    CDKデプロイ出力の`dungeon-adventure-infra-sandbox.GameApiGameApiEndpointXXX`値でURLを置き換え、リージョンを適切に設定してください。
    </Aside>
  </TabItem>
</Tabs>
